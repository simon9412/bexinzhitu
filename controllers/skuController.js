...
async function getCategoryList(req, res) {
    try {
        const categoryList = await Promise.all(CATEGORY.map(async (item) => {
            const categoryItem = await Sku.find({ category: item })
                .populate({ path: 'goodInfo', select: { _id: 0, __v: 0 } })
                .sort({ goodId: 1 })
                .select({ _id: 0, __v: 0 })
                .exec();

            const secCategories = categoryItem.map(sec => sec.goodInfo.secCategory);
            return {
                category: item,
                secCategory: [...new Set(secCategories)].sort()
            };
        }))
...
async function getSkuListByCategory(req, res) {
    ...
    try {
        ...
        const results = await Sku.find({ category })
            .populate({ path: 'goodInfo', select: { costPriceList: 0, _id: 0, __v: 0 } })
            .sort({ goodId: 1 })
            .skip(skipCount)
            .limit(pageSize)
            .select({ _id: 0, __v: 0 })
        ...
        if (secCategory) {
            const newResult = results.filter(item => item.goodInfo.secCategory === secCategory);
            return res.status(200).json({
                statusCode: statusCode.success,
                msg: '查询成功',
                data: [{ skuInfo: newResult }]
            });
        }
 ...
 * @param {number} skuInfo[].originalPriceList - 商品价格列表，每层楼价格不一样
 * @param {number} skuInfo[].costPriceList - 商品成本列表，每层楼价格不一样
    ...
    try {
        ...
        await Promise.all(skuInfo.map(async ({
            ...
            skuInfos.push({
                ...
                secCategory: goodInfo.secCategory,
 ...
 * @param {string} skuInfo[].unit - *商品单位
 * @param {string} skuInfo[].brandName - *商品品牌名称
 * @param {number} skuInfo[].originalPriceList - *商品价格
 * @param {number} skuInfo[].costPriceList - *商品成本价
 * @param {number} skuInfo[].inventory - *商品库存量
 * @return Promise
 */
async function updateSku(req, res) {
    ...
    try {
        const results = await Promise.all(skuInfo.map(async (sku) => {
            ...
            const newInfo = {
                secCategory: sku.secCategory ? sku.secCategory : existingProduct.secCategory,
    ...
    try {
        ...
        CATEGORY.map(category => {
            pipeline.push({
                ...
                categoryPrefix.push(`$${category.toLowerCase()}`)
        });
        pipeline.push(
            {
                $project: {
                    skuInfo: { $concatArrays: categoryPrefix }, // 将所有查询结果合并
                    goodId: 1, // 0不展示，1展示
                    category: 1, // 0不展示，1展示
                    _id: 0 // 0不展示，1展示
                }
            ...
            { $limit: pageSize ? parseInt(pageSize) : 100 },
            {
                $project: { // 选择要返回的字段
                    ...
                    goodId: 1,
                    goodInfo: {
                        secCategory: '$skuInfo.secCategory',
 ...
 * @return Promise
 */
async function getSkuListByAdmin(req, res) {
    const { goodId, page, pageSize } = req.query;
    // 验证page是否为大于0的数字
    if (page && (isNaN(page) || parseInt(page) <= 0)) {
        return res.status(400).json({
